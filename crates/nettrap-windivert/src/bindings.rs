...
impl WinDivert {
    ...
    pub fn recv(
        ...
        let buf_len = windivert_buffer_len(buf.len())?;
...
pub fn close_handle(handle: HANDLE) -> Result<(), String> {
    ...
    let result = unsafe { close(handle) };
    if result == 0 {
        Err(format!(
            "WinDivertClose failed (GetLastError={})",
            last_error()
        ))
    } else {
        Ok(())
...
#[cfg(test)]
mod tests {
    use super::{WindivertAddress, windivert_buffer_len};
    ...
    fn windivert_buffer_len_accepts_u32_max() {
        assert_eq!(windivert_buffer_len(u32::MAX as usize), Ok(u32::MAX));
    }

    #[test]
    fn windivert_buffer_len_rejects_values_above_u32_max() {
        if usize::BITS <= u32::BITS {
            return;
        }

        let len = u32::MAX as usize + 1;
        let err = windivert_buffer_len(len).expect_err("oversized length should be rejected");
