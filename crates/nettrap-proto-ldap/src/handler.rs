...
impl LdapHandler {
    pub fn new() -> Self {
        Self { base_dn: "dc=nettrap,dc=local".to_string() }
    }
    pub fn with_base_dn(mut self, dn: impl Into<String>) -> Self { self.base_dn = dn.into(); self }
    ...
    pub fn handle(&self, data: &[u8]) -> Vec<u8> {
        ...
        // Sequence tag 0x30
        if data[0] != 0x30 { return Vec::new(); }

        let (msg_id, op_offset) = Self::parse_message_id(data);
        if op_offset >= data.len() { return Vec::new(); }

        let op_tag = data[op_offset] & 0x1F; // Application tag number
        let op_class = (data[op_offset] >> 6) & 0x03; // Class (should be Application = 1)

        tracing::info!("LDAP message_id={}, op_tag={}, class={}", msg_id, op_tag, op_class);

        match op_tag {
            0 => { // BindRequest (tag 0, Application class)
    ...
    fn parse_message_id(data: &[u8]) -> (u32, usize) {
        ...
        if pos >= data.len() || data[pos] != 0x02 { return (0, pos); }
        pos += 1;
        let (id_len, len_bytes) = Self::parse_ber_length(&data[pos..]);
        pos += len_bytes;

        let mut msg_id = 0u32;
        for i in 0..id_len.min(4) {
            if pos + i < data.len() {
    ...
    fn extract_bind_dn(data: &[u8], op_offset: usize) -> Option<String> {
        ...
        if pos >= data.len() || data[pos] != 0x04 { return None; }
        pos += 1;
        let (dn_len, len_bytes) = Self::parse_ber_length(&data[pos..]);
        pos += len_bytes;
        if pos + dn_len > data.len() { return None; }
    ...
    fn wrap_ldap_message(&self, msg_id: u32, op_data: &[u8]) -> Vec<u8> {
        let mut msg_id_bytes = vec![0x02]; // INTEGER
        if msg_id <= 127 {
            msg_id_bytes.push(1);
...
impl Default for LdapHandler { fn default() -> Self { Self::new() } }
