...
impl HttpServer {
    ...
    pub fn add_custom_response(&self, path: impl Into<String>, response: impl Into<String>) {
        self.custom_responses.write().insert(path.into(), response.into());
    }
    ...
    fn build_response(&self, request: &HttpRequest) -> HttpResponse {
        ...
        let body = self.custom_responses
            .read()
            .get(path)
            .cloned()
...
impl HttpHandlerTrait for HttpServer {
    async fn handle_request(&self, request: HttpRequest) -> Result<HttpResponse> {
        tracing::debug!("HTTP request: {} {} {}", request.method, request.uri, request.version);
...
impl HttpRequest {
    pub fn parse(data: &[u8]) -> Result<Option<Self>> {
        let header_end = data.windows(4)
            .position(|w| w == b"\r\n\r\n")
            .ok_or_else(|| Error::Parse("No header end found".into()))?;
        
        let headers_str = std::str::from_utf8(&data[..header_end])
