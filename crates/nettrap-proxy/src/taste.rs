...
impl ProtocolTaste for TlsTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        ...
        if dst_port == 443 { return 40; }
        0
    }
    fn protocol_name(&self) -> &'static str { "tls" }
}

pub struct SmtpTaste;
impl ProtocolTaste for SmtpTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 25 || dst_port == 587 || dst_port == 465 { return 85; }
        if data.len() >= 4 {
            let upper: Vec<u8> = data[..4.min(data.len())].iter().map(|b| b.to_ascii_uppercase()).collect();
            if upper.starts_with(b"EHLO") || upper.starts_with(b"HELO") || upper.starts_with(b"MAIL") {
                return 90;
            }
        }
        0
    }
    fn protocol_name(&self) -> &'static str { "smtp" }
}

pub struct FtpTaste;
impl ProtocolTaste for FtpTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 21 { return 85; }
        if data.len() >= 4 {
            let upper: Vec<u8> = data[..4.min(data.len())].iter().map(|b| b.to_ascii_uppercase()).collect();
            if upper.starts_with(b"USER") || upper.starts_with(b"PASS") || upper.starts_with(b"LIST") {
                return 80;
            }
        }
        0
    }
    fn protocol_name(&self) -> &'static str { "ftp" }
}

pub struct Pop3Taste;
impl ProtocolTaste for Pop3Taste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 110 || dst_port == 995 { return 85; }
        if data.len() >= 4 {
            let upper: Vec<u8> = data[..4.min(data.len())].iter().map(|b| b.to_ascii_uppercase()).collect();
            if upper.starts_with(b"USER") || upper.starts_with(b"PASS") || upper.starts_with(b"STAT")
    ...
    fn protocol_name(&self) -> &'static str { "pop3" }
}

pub struct IrcTaste;
impl ProtocolTaste for IrcTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 6667 || dst_port == 6697 { return 85; }
        if data.len() >= 4 {
            let upper: Vec<u8> = data[..5.min(data.len())].iter().map(|b| b.to_ascii_uppercase()).collect();
            if upper.starts_with(b"NICK ") || upper.starts_with(b"USER ") || upper.starts_with(b"JOIN ")
               || upper.starts_with(b"PING ") || upper.starts_with(b"CAP ") {
                return 80;
            }
        }
        0
    }
    fn protocol_name(&self) -> &'static str { "irc" }
}

pub struct TftpTaste;
impl ProtocolTaste for TftpTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 69 { return 90; }
        if data.len() >= 4 {
    ...
    fn protocol_name(&self) -> &'static str { "tftp" }
}
...
impl ProtocolTaste for TelnetTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 23 { return 90; }
        // Telnet IAC sequences start with 0xFF
        if data.len() >= 3 && data[0] == 0xFF { return 80; }
        // SSH- prefix means it's SSH, not telnet
        if data.starts_with(b"SSH-") { return 0; }
        0
    }
    fn protocol_name(&self) -> &'static str { "telnet" }
}

pub struct SshTaste;
impl ProtocolTaste for SshTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if data.starts_with(b"SSH-") { return 95; }
        if dst_port == 22 { return 85; }
        0
    }
...
impl ProtocolTaste for SmbTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 445 || dst_port == 139 { return 85; }
        // NetBIOS + SMB magic
        if data.len() >= 8 && data[0] == 0x00 && (data[4..8] == *b"\xffSMB" || data[4..8] == *b"\xfeSMB") { return 95; }
        if data.len() >= 4 && (data[0..4] == *b"\xffSMB" || data[0..4] == *b"\xfeSMB") { return 95; }
        0
    }
    fn protocol_name(&self) -> &'static str { "smb" }
}

pub struct RdpTaste;
impl ProtocolTaste for RdpTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 3389 { return 90; }
        // TPKT header: version 3
        if data.len() >= 4 && data[0] == 0x03 && data[1] == 0x00 { return 70; }
        0
    }
    fn protocol_name(&self) -> &'static str { "rdp" }
}

pub struct RedisTaste;
impl ProtocolTaste for RedisTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 6379 { return 90; }
        let text = String::from_utf8_lossy(data).to_uppercase();
        if text.starts_with("*") || text.starts_with("PING") || text.starts_with("INFO") || text.starts_with("AUTH") { return 80; }
        0
    }
    fn protocol_name(&self) -> &'static str { "redis" }
}

pub struct MysqlTaste;
impl ProtocolTaste for MysqlTaste {
    fn taste(&self, _data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 3306 { return 90; }
        0
    }
...
impl ProtocolTaste for MqttTaste {
    ...
    fn protocol_name(&self) -> &'static str { "mqtt" }
}

pub struct SnmpTaste;
impl ProtocolTaste for SnmpTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 161 || dst_port == 162 { return 90; }
        if data.len() >= 5 && data[0] == 0x30 && data[2] == 0x02 { return 60; }
        0
    }
    fn protocol_name(&self) -> &'static str { "snmp" }
}

pub struct SocksTaste;
impl ProtocolTaste for SocksTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 1080 { return 85; }
        if data.len() >= 3 && (data[0] == 0x04 || data[0] == 0x05) { return 70; }
        0
    }
    fn protocol_name(&self) -> &'static str { "socks" }
}

pub struct MemcachedTaste;
impl ProtocolTaste for MemcachedTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 11211 { return 90; }
        let text = String::from_utf8_lossy(data);
        if text.starts_with("get ") || text.starts_with("set ") || text.starts_with("stats") || text.starts_with("version") { return 85; }
        if data.len() >= 24 && data[0] == 0x80 { return 75; } // binary protocol
        0
    }
    fn protocol_name(&self) -> &'static str { "memcached" }
}

pub struct PostgresTaste;
impl ProtocolTaste for PostgresTaste {
    fn taste(&self, _data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 5432 { return 90; }
        0
    }
    fn protocol_name(&self) -> &'static str { "postgres" }
}

pub struct SipTaste;
impl ProtocolTaste for SipTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 5060 || dst_port == 5061 { return 85; }
        if data.starts_with(b"SIP/") || data.starts_with(b"REGISTER") || data.starts_with(b"INVITE") || data.starts_with(b"OPTIONS") { return 90; }
        0
    }
    fn protocol_name(&self) -> &'static str { "sip" }
}

pub struct UpnpTaste;
impl ProtocolTaste for UpnpTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 1900 { return 85; }
        if data.starts_with(b"M-SEARCH") || data.starts_with(b"NOTIFY") { return 90; }
        0
    }
    fn protocol_name(&self) -> &'static str { "upnp" }
}

pub struct NtpTaste;
impl ProtocolTaste for NtpTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 123 { return 90; }
        if data.len() == 48 || data.len() == 68 { return 60; }
        0
    }
    fn protocol_name(&self) -> &'static str { "ntp" }
}

pub struct CoapTaste;
impl ProtocolTaste for CoapTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port == 5683 || dst_port == 5684 { return 90; }
        if data.len() >= 4 && (data[0] >> 6) == 1 { return 50; }
        0
    }
    fn protocol_name(&self) -> &'static str { "coap" }
}

pub struct NknTaste;
impl ProtocolTaste for NknTaste {
    fn taste(&self, data: &[u8], dst_port: u16) -> TasteScore {
        if dst_port >= 30001 && dst_port <= 30003 { return 85; }
        let text = String::from_utf8_lossy(data);
        if text.contains("\"jsonrpc\"") && (text.contains("getnodestate") || text.contains("getlatestblockheight")) { return 90; }
        0
    }
...
impl ProtocolTaste for RawTaste {
    ...
    fn protocol_name(&self) -> &'static str { "raw" }
}
