...
After running, NetTrap generates:
- `events.jsonl` — Real-time event stream
- `events_nbi.jsonl` — Network Behavior Indicators
- `events_nbi.html` — HTML report with visualizations
- `events_nbi.sarif.json` — SARIF for security tool integration
- `events_nbi.csv` — Spreadsheet-compatible export
