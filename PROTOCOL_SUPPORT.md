...
|---|---|---:|---|
| DNS | Parses UDP/TCP queries and builds synthetic records/NXDOMAIN responses | Yes (`dig`, UDP) | TCP client E2E and broad resolver compatibility are not required gates |
| HTTP | Parses requests, records metadata/body length, serves default/custom/webroot responses | Yes (`curl`) | Not a production web server; browser compatibility is not a required gate |
| TLS | Detects ClientHello, fingerprints it, and can terminate inbound TLS locally | No | No general upstream MITM, selective passthrough, or pinning bypass |
...
| IMAP/IMAPS | Explicit listener banner and command subset | No | No content detector; must be selected by listener name |
