...
Every NetTrap release archive, Linux package, and Windows MSI has a keyless
Sigstore bundle, a GitHub artifact attestation, and an SBOM attestation. The
draft release is not published unless the workflow verifies every Linux/macOS
`.tar.gz`, Linux `.deb`/`.rpm`, and Windows `.zip`/`.msi` against this
