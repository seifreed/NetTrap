# -*- perl -*-
#
# INetSim::Quotd::UDP - A fake UDP quotd server
#
...
sub configure_hook {
    ...
    $self->{server}->{port}   = INetSim::Config::getConfigParameter("Quotd_UDP_BindPort");  # bind to port
...
#    $self->{server}->{udp_recv_len} = 1024;                                                 # default is 4096
    ...
    $self->{servicename} = INetSim::Config::getConfigParameter("Quotd_UDP_ServiceName");
    $self->{max_childs} = INetSim::Config::getConfigParameter("Default_MaxChilds");

    my ($author, $quote) = INetSim::Quotd::select_quote(INetSim::Config::getConfigParameter("Quotd_UDP_ServiceName"));
    $self->{logtext} = "$quote  ($author)";
    $quote =~ s/(.{1,72})(\s+|\z)|(.{1,72})/$1\r\n/gm;
    $self->{outtext} = "$quote\r\n-- $author\r\n";
}
